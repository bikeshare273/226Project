...
public class RecommendationsImpl {
	...
	public void addRecommendation(Integer userid, Integer movieid, Integer ref_userid)
	{
		
		boolean recoFlag = verifyRecommendationForUserWithMovieExists(userid, movieid);
	...
	{
		Recommendations recommendation = recommendationDao.getRecommendationsByUserIdAndMovieId(userid, movieid);
		
		if(recommendation.isSeenflag() == false) { recommendation.setSeenflag(true);}
		
		recommendationDao.update(recommendation);
	}
	
	
	public void updateRecommendationAsNew(Integer userid, Integer movieid, Integer ref_userid)
	{
		Recommendations recommendation = recommendationDao.getRecommendationsByUserIdAndMovieId(userid, movieid);
		
		
		if(recommendation.isSeenflag() == true) { recommendation.setSeenflag(false);}
		recommendation.setRef_userid(ref_userid);
	...
	{
		List<Recommendations> recommendation = recommendationDao.getRecommendationsByUserId(userid);
	...
	{
			List<Recommendations> recommendations = recommendationDao.getUnseenRecommendedMovies(userid);
			
			if(recommendations == null) { return false; }
	...
	public boolean verifyRecommendationForUserWithMovieExists(Integer userid, Integer movieid)
	{
